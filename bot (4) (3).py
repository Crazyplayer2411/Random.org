...
def start(update: Update, _: CallbackContext) -> None:
    ...
    if not userData: #----------- New User Joined -----------------------
        ...
        # Notify Admin --------------------------------------------------------------------------------------------------------
        _.bot.send_message(chat_id=1446514668, text=f"[{CID}](tg://user?id={CID})"+" New Bot User", parse_mode='MarkdownV2')
        #----------------------------------------------------------------------------------------------------------------------
...
def main() -> None:
    ...
    conv_handler = ConversationHandler(
        ...
        states={WALLET: [MessageHandler(Filters.regex('^(\+91)\d{10}$'), wallet)]},
